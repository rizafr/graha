...
<script type="text/javascript">
	...
	{
		loading();
		$('#frame_data').load(base_url+'report/promo_list/'+posisi);
	}
	...
	function list_filter(posisi)
	{
		...
		else
		{
			if(filter == "nomor_pemesanan")
			{
				...
				{
					loading();
					$('#frame_data').load(base_url+'report/promo_cari_list/'+posisi+'/'+filter+'/0/0/'+kata_kunci);
				}
			}	
			else if(filter == "cluster")
			{
				loading();
				$('#frame_data').load(base_url+'report/promo_cari_list/'+posisi+'/'+filter+'/'+id_cluster+'/'+kategori+'/0');
			}			
		}
	}
	...
	function hapus_pemesanan(id_pemesanan, id_unit, posisi){
		if(confirm("Yakin akan menghapus data?")){
			$.post(base_url+'report/promo_delete/'+id_pemesanan+'/'+id_unit, function(){
...
<script type="text/javascript">
	$(document).ready(function() {	
		$('#frame_data').load(base_url+'report/promo_list/0');
	});
</script>
