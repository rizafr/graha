...
function display_upload_form()
{
	?>
		<form action="uploader.php?<?php echo $_SERVER["QUERY_STRING"]; ?>&q=upload" method="post" enctype="multipart/form-data" onSubmit="">
			...
			<!-- <input type="file" size="70" name="upload_file" ID="File1" accept="image/*"> -->
			<br/>
...
function upload_content_file($DestPath, $DestLinkPath)
{
	...
	//validasi extensi
	$nama_file=$FileObject['name'];
	$ex=substr($nama_file,-3,3);
	if($ex!="jpg"&&$ex!="jpeg"&&$ex!="JPG"&&$ex!="gif"&&$ex!="swf"&&$ex!="png"&&$ex!="bmp"){
		$StatusMessage = "File harus berupa gambar atau flash (.swf)";
	} else {
	if(!isset($FileObject) || $FileObject["size"]<=0)
	{		
		$StatusMessage = "Gagal! Tidak ada file yang dipilih!";		
	}	
	else
	{	
		...
		{
			if($ReplaceFile=="yes")
				$StatusMessage .= "Menimpa file yang sudah ada. ";										// if so, we'll let the user know
			else
				$StatusMessage . "Gagal! File '$ActualFileName' sudah ada!";
		}
		if($ReplaceFile=="yes")
		{
			if($ResizeSizeX!=0 || $ResizeSizeY!=0)														// if we need to resize the file
			{
				...
				{
					$StatusMessage .= "Tidak bisa mengganti ukuran file!. ";
					move_uploaded_file($FileObject['tmp_name'], $ActualFileName);
				}
			}
			else
				move_uploaded_file($FileObject['tmp_name'], $ActualFileName);
			$StatusMessage .=  "File: " . $FileObject['name'] . " telah berhasil di upload!";		// to " . $ActualFileName . "!";						
